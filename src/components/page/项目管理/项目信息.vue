	...
	<div>
		<el-form class="main" :model="form" >
					...
					<el-form :model="form.projectInfoVo" :label-position="labelPosition" label-width="120px" :rules="rules" ref="projectInfoVo">
						...
						<div class="left">
							<el-form-item label="项目名称 :" prop="name">
								<el-input v-model="form.projectInfoVo.name" placeholder="请输入项目名称" style="width: 300px;"></el-input>
							</el-form-item>
							...
							<el-form-item label="详细地址:" prop='address'>
								<el-input v-model="form.projectInfoVo.address"  placeholder="请输入详细地址" style="width: 300px;"></el-input>
							</el-form-item>
						...
						<div class="right">
							...
							<el-form-item label="经度 :" prop='latitudes'>
								<el-input v-model="form.projectInfoVo.latitudes" placeholder="请输入经度" style="width: 200px;float: left;" disabled></el-input>
								<img src="../../../assets/img/dw.png" style="width: 30px;height: 30px;display: block;float: left;margin-left: 20px;"  @click="location"/>
							</el-form-item>
							<el-form-item label="纬度 :" prop='longitudes'>
								<el-input v-model="form.projectInfoVo.longitudes" placeholder="请输入纬度" style="width:200px;" disabled></el-input>
							</el-form-item>
			...
			<div class="info">
				...
				<div class="ff1">
					<el-form :model="form.controllRespDto" :label-position="labelPosition" label-width="120px">
						...
						<el-form-item label="当前状态 :" style="width: 900px;">
							...
							<el-checkbox label="暂停销售" v-model="form.controllRespDto.stopSell" v-show="false"></el-checkbox>
	    					<el-checkbox label="是否控制用户扫码查看项目" v-model="form.controllRespDto.local" v-show="false"></el-checkbox>
				...
				<div class="ff1">
					<el-form :model="form.controllRgou" :label-position="labelPosition" label-width="120px" :rules="rule" ref="controllRgou">
						...
						<el-form-item label="请设置认购金 :" prop='buyMoney'>
							<el-input v-model="form.controllRgou.buyMoney" placeholder="请输入认购金" style="width: 200px;" @blur.prevent="buyMoneys"></el-input>&nbsp;&nbsp;元
						</el-form-item>
						<el-form-item label="可认购开始时间 :">
							<el-date-picker v-model="form.controllRgou.buyStartTime" type="datetime" placeholder="选择日期时间" v-if='form.controllRgou.show3'></el-date-picker>
						...
						<div class="text">
							...
							<el-input v-model="signTime" placeholder="请输入时间" style="width: 150px;margin-right: 5px;"></el-input>
							...
							<span style="color: black;">小时</span>，后未到线下进行签约付尾款，则取消认购订单
						</div>
					</el-form>
				</div>
			</div>
			...
			<div class="info">
				...
				<div class="ff1">
					<el-form :model="form.controllDownSpace" :label-position="labelPosition" label-width="120px" :rules="rule4" ref="controllDownSpace">
						...
						<div class="choise">
							...
							<div style="margin-top: 20px;margin-bottom: 20px;">
								<el-checkbox label="失效后退还到钱包的落位金需要解冻" v-model="form.controllDownSpace.downMoneyUnfreeze" :checked="checked8==1" @change="change8"></el-checkbox>
							</div>
						</div>
						<el-form-item label="请设置落位金 :" prop='downMoney'>
							<el-input v-model="form.controllDownSpace.downMoney" placeholder="请输入落位金" style="width: 200px;" @blur.prevent="downMoneys"></el-input>&nbsp;&nbsp;元
						</el-form-item>
						...
						<el-form-item label="一个车位可 :" prop='downLimitNum'>
							<el-input v-model="form.controllDownSpace.downLimitNum" placeholder="请输入人数" style="width: 200px;"></el-input>&nbsp;&nbsp;人落位
						</el-form-item>
						<el-form-item label="缴纳一次落位金可落 :" prop='downCarspaceNum' label-width="160px">
							<el-input v-model="form.controllDownSpace.downCarspaceNum"  placeholder="请输入个数" style="width: 200px;"></el-input>&nbsp;&nbsp;个车位
						</el-form-item>
						<div class="text">
							落位后，线下签订合约的时间开始计时，若超过
							<el-input v-model="downExpireTime" placeholder="请输入时间" style="width: 150px;margin-right: 5px;"></el-input>
							<span style="color: black;">小时</span>， 后未到线下进行签约付尾款，则取消落位订单
						</div>
					</el-form>
				</div>
			</div>
			...
			<div class="info">
				<div class="title">
					<span class="f1"></span>置业顾问管理
					<el-button style="float: right;" @click="addMember" class="button">新增成员</el-button>
				</div>
				<!--表格列表-->
				<div class="ff1">
						...
						<el-table :data="form.consultantVo" border :header-cell-style="{background:'#dcd3ef',color:'#606266'}"  header-row-class-name="tableHead">
							...
							<el-table-column  label="状态" align="center" width="150">
								<template slot-scope="scope">
									<div>
										<span @click="lineChange(form.consultantVo[scope.$index].id,scope.$index)" v-if='form.consultantVo[scope.$index].status==1' style='color:#f00;cursor: pointer;'>{{form.consultantVo[scope.$index].cstatus}}</span>
										<span @click="lineChange(form.consultantVo[scope.$index].id,scope.$index)" v-else style='color:#ccc;cursor: pointer;'>{{form.consultantVo[scope.$index].cstatus}}</span>
									</div>
								</template>
							</el-table-column>
							<el-table-column prop="caozuo" label="操作" align="center" width="348">
								<template slot-scope="scope">
				                        ...
				                        <a style="margin-left: 20px;color:#f00;" @click="deleteInfo(scope.$index)" v-if="scope.$index!=0">删除</a>
				                </template>
							</el-table-column>
						</el-table>
					</div>
        ...
        <el-dialog title="新增成员" :visible.sync="revise" width="25%" >
            <el-form ref="info" :model="info" :rules='rule5' :label-position="labelPosition" label-width="120px">
                ...
                <el-form-item label="手机号:" prop='phone'>
                    <el-input v-model="info.phone" placeholder="请输入需要添加人员的手机号" @blur="jugePhone(info.phone)"></el-input>
                </el-form-item>
        ...
        <el-dialog title="修改成员" :visible.sync="editVisible" width="25%"> 
            <el-form ref="info1" :model="info1" :label-position="labelPosition" label-width="150px" :rules='rules1' >
                <el-form-item label="置业顾问名称:" prop='name'>
                    <el-input v-model="info1.name" placeholder="请输入需要添加人员的姓名"></el-input>
                </el-form-item>
                <el-form-item label="手机号:" prop="phone">
                    <el-input v-model="info1.phone" placeholder="请输入需要添加人员的手机号" @blur="jugePhone(info1.phone)"></el-input>
                </el-form-item>
                <el-form-item v-show="false">
            ...
            <span slot="footer" class="dialog-footer">
            	<el-button type="primary" @click="saveEdit('info1')">保存</el-button>
                <el-button @click="editVisible=false">取 消</el-button> 
            </span>
        </el-dialog>
        
        <!-- 地图 -->
    <el-dialog title="百度地图" :visible.sync="dialogVisible" center class="baiduSearch">
	...
	export default{
		...
		data(){
	        return {
				...
				rule4:{
					downMoney:{required:true,message:'请设置落位金',trigger:'blur'},
					downLimitNum:{required:true,message:'请设置几人可落一个车位',trigger:'blur'},
					downCarspaceNum:{required:true,message:'请设置1人可落几个车位',trigger:'blur'},
				},
				rule5:{
					name:[
	    ...
	    methods:{
			jugePhone(phone){
				...
				// 	// }
				// 	if(phone!==''){
				// 		console.log(isvalidPhone.test(phone))
		    ...
		    infohtmlset(poi){
		      ...
		      this.form.projectInfoVo.region=poi.province;
		      this.form.projectInfoVo.city=poi.city;
		      this.form.projectInfoVo.arr='';
		      // 保存到页面data中
		      this.form.projectInfoVo.address=detalAddress;
		      this.form.projectInfoVo.latitudes=poi.point.lat
		      this.form.projectInfoVo.longitudes=poi.point.lng
			...
			renderBasicInfo(){
				...
				.then(res=>{
					if(res.data.code==0){
						...
						this.headImage=res.data.data.projectInfoVo.headImage
						this.cityChange();
						// 置业顾问
...
//							});
							...
							//有数据就修改
							// 认购名称
								if(res.data.data.controllRespDto.useBuyName==null||res.data.data.controllRespDto.useBuyName==1){
									...
									this.form.controllRgou.buyName=res.data.data.controllRespDto.buyName;
									this.form.controllRgou.show1=true;
								}
								// 认购金名称
								if(res.data.data.controllRespDto.useBuyMoneyName==null||res.data.data.controllRespDto.useBuyMoneyName==1){
									...
									this.form.controllRgou.buyMoneyName=res.data.data.controllRespDto.buyMoneyName;
								}
								// 开盘时间是否一致
...
//									this.change3();
							...
							this.form.controllRgou.buyMoney=res.data.data.controllRespDto.buyMoney;
							//认购时间
							this.signTime=(parseFloat(res.data.data.controllRespDto.buyExpireTime/3600)).toFixed(2);
						}
						//项目落位数据
						this.form.controllDownSpace=res.data.data.controllRespDto;
						
						//落位时间
						this.downExpireTime=(parseFloat(res.data.data.controllRespDto.downExpireTime/3600)).toFixed(2);
						//允许用户落位 checked5
						...
						//使用落位金名称
						if(res.data.data.controllRespDto.useDownMoneyName==null||res.data.data.controllRespDto.useDownMoneyName==1){
							this.form.controllDownSpace.useDownMoneyName=true;
							this.checked6=1;
						}else{
							this.form.controllDownSpace.useDownMoneyName=false;
							this.checked6=0;
							this.form.controllDownSpace.downMoenyName=res.data.data.controllRespDto.downMoenyName;
						}
						//是否取消订单
...
//						})
			...
			//认购金判断
			buyMoneys(){
				if(parseInt(this.form.controllRgou.buyMoney)<0){
	    			this.$message({
						type: 'info',
						message: '认购金需大于等于0！'
					});
					this.form.controllRgou.buyMoney=''
					return
	    		}
			},
            //落位金判断
			downMoneys(){
				if(parseInt(this.form.controllDownSpace.downMoney)<0){
	    			this.$message({
						type: 'info',
						message: '落位金需大于等于0！'
					});
					this.form.controllDownSpace.downMoney=''
					return
	    		}
			},
			//省份
...
//			},
			...
			saveBasicInfo(projectInfoVo){
				this.$refs.projectInfoVo.validate((valid) => {
					if (valid) {
						if(this.headImage==''||this.headImage==null){
							this.$message({
								type: 'info',
								message: '请上传项目图片！'
							});
						...
						params.append('headImage',this.headImage);
			...
			//上线下切换
			lineChange(id,i){
				if(this.form.consultantVo.length==1){
					this.$message({
						type: 'info',
						message: '只有一个置业顾问时，不能点击下线！'
					});
					return
				}
			...
			//新增成员保存
	    	saveInfos(info){
				// let bool=this.jugePhone(info.phone)
				// console.log(bool);
			...
			saveControl(){
				...
				params.append('startTime',this.transformDate(this.form.controllRespDto.startTime));
				params.append('endTime',this.transformDate(this.form.controllRespDto.endTime));
...
//						this.reload();
			...
			selectChange6(val){
				...
				if(val==0){
					...
					this.form.day1=parseInt(this.downExpireTime)*24;
				}	
			},
			//认购控制保存
					...
					if (valid) {
						...
						if(this.form.controllRespDto.startTime==''||this.form.controllRespDto.startTime==undefined||this.form.controllRespDto.startTime==null){
							...
							if(this.form.controllRgou.checked3==true){
								...
								params.append('buyStartTime',this.transformDate(this.form.controllRgou.buyStartTime));
								params.append('startTime',this.transformDate(this.form.controllRespDto.startTime));
								params.append('endTime',this.transformDate(this.form.controllRespDto.endTime));
							}
						}
						...
						//小时 天处理传参
						//1为天
						this.form.day=parseInt(this.signTime*3600);
...
//									this.reload()
			...
			//落位认购保存
			saveDownSpace(controllDownSpace){
//				if(this.form.controllRespDto.startTime==''||this.form.controllRespDto.startTime==undefined||this.form.controllRespDto.startTime==null){
//					this.$message({
//						type:'error',
//						message:'请先选择开盘时间并保存！'
...
//				}
				...
				this.$refs.controllDownSpace.validate((valid) => {
					if (valid) {
						var params=new URLSearchParams();
//						params.append('startTime',this.transformDate(this.form.controllRespDto.startTime));
//						params.append('endTime',this.transformDate(this.form.controllRespDto.endTime));
						...
						if(this.form.controllDownSpace.buyFollowOpen==true){
							...
							if(this.form.controllDownSpace.downStartTime!=undefined||this.form.controllDownSpace.downEndTime!=undefined){
						...
						//1为天
						this.form.day1=parseInt(this.downExpireTime*3600);
