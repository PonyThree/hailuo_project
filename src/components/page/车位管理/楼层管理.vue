		...
		<el-form :model="form" class="chess">
			<el-form-item label="状态:" style="width: 30%;float: left;margin-left: 50px;">
                <el-select v-model="form.status" placeholder="请选择状态" style="width: 200px;">
				     <el-option key="3" label="所有状态" value="3"></el-option>
            ...
            <el-form-item style="width: 40%;float: right;margin-right: 50px;">
                <el-input v-model="form.name" placeholder="请输入楼层名称" style="width: 300px;float: right;"></el-input>
            </el-form-item> 
		</el-form>
		<div class="table">
			<!--按钮-->
			<ul class="listTwo">
				<li><el-button  type="primary" @click="addTag">新增楼层</el-button></li>
			...
			<el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" border style="width: 100%;" @selection-change="handleSelectionChange" :header-cell-style="{background:'#dcd3ef',color:'#606266'}"  header-row-class-name="tableHead">
				...
				<!-- <el-table-column prop="image" label="车位分布图" align="center" width="300">
					<template slot-scope="scope">
						<img :src="scope.row.image"  style="width: 280px;height: 100px;border: none;"/>
					</template>
				</el-table-column> -->
	...
	export default{
	    ...
	    methods:{
	    	...
	    	checked(currentPage){
				...
				this.$axios.get(request.testUrl+"/product/auth1/TruckSpaceLevelThree/selectList?page="+this.currentPage+"&pageSize="+this.pagesize,{
					params:{
				...
				.then(res=>{
					if(res.data.code==0){
						...
						// 	if(this.tableData[i].truckSpaceImageList.length!=0){
	    	...
	    	batchDelete(){
	    		...
	    		this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
					...
					this.$axios({
			        	method:'post',
			        	url:request.testUrl+"/product/auth1/TruckSpaceLevelThree/doDeleteBath",
			        	data:id
					}).then(res=>{
						if(res.data.code==0){
							this.checked()
	      	...
	      	//新增区域按钮
	      	addTag(){
	      		this.$router.push({ path: '/新增楼栋管理-第三级'})
	      	},
	      	//修改
	      	modifyTable(index){
	      		this.$router.push({ path: '/修改楼栋管理-第三级',query:{id:this.tableData[index].id}})
	      	},
	      	//删除每一行
	      	deleteTable(index){
        		 this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
		    ...
		    handleCurrentChange(currentPage) {
		        this.checked(currentPage); //点击第几页
		    },
