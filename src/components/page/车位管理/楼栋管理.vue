	...
	<div>
		<el-form :model="form" class="chess">
            ...
            <el-form-item style="width: 40%;float: right;margin-right: 50px;">
                <el-input v-model="form.name" placeholder="请输入楼栋名称" style="width: 300px;float: right;"></el-input>
            </el-form-item> 
		</el-form>
		<div class="table">
			<!--按钮-->
			<ul class="listTwo">
				<li><el-button  type="primary" @click="addTag">新增楼栋</el-button></li>
			...
			<el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" border style="width: 100%;" @selection-change="handleSelectionChange" :header-cell-style="{background:'#dcd3ef',color:'#606266'}"  header-row-class-name="tableHead">
				...
				<el-table-column prop="name" label="楼栋" align="center"></el-table-column>
	...
	export default{
	    ...
	    methods:{
	    	...
	    	checked(currentPage){
				...
				this.$axios.get(request.testUrl+"/product/auth1/TruckSpaceLevelTwo/selectList?page="+this.currentPage+"&pageSize="+this.pagesize,{
					params:{
	    	...
	    	batchDelete(){
	    		...
	    		this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
					...
					this.$axios({
			        	method:'post',
			        	url:request.testUrl+"/product/auth1/TruckSpaceLevelTwo/doDeleteBath",
			        	data:id
					}).then(res=>{
						if(res.data.code==0){
							this.checked()
	      	...
	      	//新增区域按钮
	      	addTag(){
	      		this.$router.push({ path: '/新增楼栋管理-第二级'})
	      	},
	      	//修改
	      	modifyTable(index){
	      		this.$router.push({ path: '/修改楼栋管理-第二级',query:{id:this.tableData[index].id}})
	      	},
	      	//删除每一行
	      	deleteTable(index){
        		 this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
