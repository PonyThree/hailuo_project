	        ...
	        imgPreview(file, callback) { 
				...
				if (/^image/.test(file.type)) {
					...
					reader.onloadend = function() { 
						...
						img.onload = function() {
							let data = self.$handlePic.compress(img);self.imgUrl = result; 
	        ...
	        save(info){
	        	this.$refs.info.validate((valid) => {
					if (valid) {
			        	...
			        	this.$axios({
				        	method:'post',
				        	url:request.testUrl+"/product/auth1/truckSpaceArea/insert",
				        	data:{
				        		"code": '',
							    "id": "",
							    ...
							    "projectId": "",
							    "showSelled": this.form.info.showSelled,
							    "showStrategy": this.form.info.showStrategy,
							    "timeFollowOpenTime":this.form.info.timeFollowOpenTime==true?1:0,
							    "stopSell": this.form.info.stopSell||false  //如果没有点暂停销售  默认false
				        	}
						}).then(res=>{
								if(res.data.code==0){
