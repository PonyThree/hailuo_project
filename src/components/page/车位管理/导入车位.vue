	...
	export default{
		...
		data(){
	        return {
	        	labelPosition:'right',
	        	form:{},
				url:request.testUrl+"/product/truckSpace/downImportTemplate",
				off:false,
	        }
		},
	    methods:{
	    	...
	    	beforeUpload(file){
			    ...
			    this.$axios.post(request.testUrl+"/product/auth1/truckSpace/importExcel",fd)
			    	.then(res=>{
			            if(res.data.code==0){
			            	this.$message({
		                        type: 'info',
